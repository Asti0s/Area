...
@secure_endpoint
async def get_subscribed_areas(
...
async def get_all_areas(token: HTTPAuthorizationCredentials = Depends(auth_scheme)):
    ...
    for area in areas:
        area["action_service"] = (await get_actions_by_field("name", area["action"]))[
            0
        ].service
        area["reaction_service"] = (
            await get_reactions_by_field("name", area["reaction"])
        )[0].service
    return {"areas": areas}
...
async def subscribe_to_area(
    area_id: str, token: HTTPAuthorizationCredentials = Depends(auth_scheme)
):
