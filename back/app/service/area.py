...
async def get_actions_service():
    ...
    for action_member in action_members:
        if (
            isinstance(action_member[1], type)
            and issubclass(action_member[1], IAction)
            and action_member[0] != "IAction"
        ):
...
async def get_reactions_service():
    """return all the currently available reactions as a list

    Returns:
        List[class]: a list of all available reactions
    """
    result = []

    # fetch all class present in the reactions.py file
    reaction_members = getmembers(reactions)

    for reaction_member in reaction_members:
        # discard any reactions that is not inheriting from the IReaction interface
        if (
            isinstance(reaction_member[1], type)
            and issubclass(reaction_member[1], IReaction)
            and reaction_member[0] != "IReaction"
        ):
