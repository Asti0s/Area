...
async def update_actions():
    """periodically check all areas for possible reactions"""
    areas = await DAO.find_all_areas()
    for area in areas:
        try:
            action_func = (await get_actions_by_field("name", area["action"]))[
                0
            ].is_triggered
            reaction_func = (await get_reactions_by_field("name", area["reaction"]))[
                0
            ].react
