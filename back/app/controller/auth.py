...
oauth.register(
    ...
    access_token_params=None,
    client_kwargs={
        "scope": "read_user read_api read_repository read_registry write_repository write_registry"
    },
)
...
async def login(credentials: Credentials, authorize: AuthJWT = Depends()):
    access_token = await login_user(
        credentials.username, credentials.password, authorize
    )
    if access_token:
