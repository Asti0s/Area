...
async def get_all_users(token: HTTPAuthorizationCredentials = Depends(auth_scheme)):
    ...
    for i, user in enumerate(users):
        ...
        users[i] = {
            "_id": user["_id"],
            "subscribed_areas": user["subscribed_areas"],
...
async def update_username(
    username: str, token: HTTPAuthorizationCredentials = Depends(auth_scheme)
):
    try:
...
async def update_password(
    password: str, token: HTTPAuthorizationCredentials = Depends(auth_scheme)
):
    try:
        user = await DAO.find_user_by_username(TokenManager.get_token_subject(token))
...
async def update_email(
    email: str, token: HTTPAuthorizationCredentials = Depends(auth_scheme)
):
    try:
        user = await DAO.find_user_by_username(TokenManager.get_token_subject(token))
        user["email"] = email
