...
async def link_to_google(username, google_token):
    ...
    google_user = await DAO.find(
        get_database().google_users, "token.access_token", google_token
    )
    ...
    await DAO.update(
        get_database().google_users, "token.access_token", google_token, google_user
    )
...
async def link_to_discord(username, discord_token):
    ...
    discord_user = await DAO.find(
        get_database().discord_users, "token.access_token", discord_token
    )
    ...
    await DAO.update_user(user["username"], user)
    await DAO.update(
        get_database().discord_users, "token.access_token", discord_token, discord_user
    )
...
async def link_to_spotify(username, spotify_token):
    ...
    spotify_user = await DAO.find(
        get_database().spotify_users, "token.access_token", spotify_token
    )
    ...
    await DAO.update_user(user["username"], user)
    await DAO.update(
        get_database().spotify_users, "token.access_token", spotify_token, spotify_user
    )
...
async def link_to_github(username, github_token):
    ...
    github_user = await DAO.find(
        get_database().github_users, "token.access_token", github_token
    )
    ...
    await DAO.update_user(user["username"], user)
    await DAO.update(
        get_database().github_users, "token.access_token", github_token, github_user
    )
...
async def link_to_gitlab(username, gitlab_token):
    ...
    gitlab_user = await DAO.find(
        get_database().gitlab_users, "token.access_token", gitlab_token
    )
    ...
    await DAO.update_user(user["username"], user)
    await DAO.update(
        get_database().gitlab_users, "token.access_token", gitlab_token, gitlab_user
    )
...
async def oauth_google_login(google_token):
    ...
    user_account = await DAO.find(
        get_database().google_users, "email", user_info["email"]
    )

    if user_account is None:
        await DAO.insert(
            get_database().google_users,
            {"email": user_info["email"], "token": google_token, "linked_to": None},
        )
...
async def oauth_gitlab_login(gitlab_token):
    async with aiohttp.ClientSession() as session:
        async with session.get(
            ...
            headers={"Authorization": f'Bearer {gitlab_token["access_token"]}'},
        ) as response:
            user_info = await response.json()
            if not user_info:
...
async def area_oauth_discord_login(discord_token):
    ...
    if discord_account is None:
        ...
        discord_account = await DAO.find(
            get_database().discord_users, "email", user_info.get("email")
        )

    elif discord_account["linked_to"] is not None:
        linked_account = await DAO.find(
            get_database().users, "_id", discord_account["linked_to"]
        )
    ...
    if linked_account is None:
        ...
        await DAO.update(
            get_database().discord_users, "email", user_info["email"], discord_account
        )
...
async def area_oauth_google_login(google_token):
    ...
    if google_account is None:
        ...
        linked_account = await DAO.find(
            get_database().users, "_id", google_account["linked_to"]
        )
    ...
    if linked_account is None:
        ...
        await DAO.update(
            get_database().google_users, "email", user_info["email"], google_account
        )
...
async def area_oauth_spotify_login(spotify_token):
    ...
    if spotify_account is None:
        ...
        spotify_account = await DAO.find(
            get_database().spotify_users, "email", user_info.get("email")
        )

    elif spotify_account["linked_to"] is not None:
        linked_account = await DAO.find(
            get_database().users, "_id", spotify_account["linked_to"]
        )
    ...
    if linked_account is None:
        ...
        await DAO.update(
            get_database().spotify_users, "email", user_info["email"], spotify_account
        )
