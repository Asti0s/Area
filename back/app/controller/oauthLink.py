...
async def link_github(
    github_token, token: HTTPAuthorizationCredentials = Depends(auth_scheme)
):
    try:
...
async def link_discord(
    discord_token, token: HTTPAuthorizationCredentials = Depends(auth_scheme)
):
    try:
    ...
    try:
        ...
        if client_type == "mobile":
            return RedirectResponse(
                f"{Config.MOBILE_URL}?discord_token={discord_token}"
            )
...
async def link_google(
    google_token, token: HTTPAuthorizationCredentials = Depends(auth_scheme)
):
    try:
...
async def link_spotify(
    spotify_token, token: HTTPAuthorizationCredentials = Depends(auth_scheme)
):
    try:
...
async def spotify_callback(request: Request):
    try:
        ...
        if client_type == "mobile":
            return RedirectResponse(
                f"{Config.MOBILE_URL}?spotify_token={spotify_token}"
            )
...
async def link_gitlab(
    gitlab_token, token: HTTPAuthorizationCredentials = Depends(auth_scheme)
):
    try:
